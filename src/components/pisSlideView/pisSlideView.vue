...
<script>
  ...
  export default {
    name: 'pisSlideView',
    components: {FileChange, LabelInfo},
    data() {
      return {
        kfbView: {},
    ...
    computed: {
      ...
      labelLength() {
        if (this.kfbView && this.kfbView.setLabelList) {
          this.kfbView.setLabelList(this.labelList);
        }
    ...
    methods: {
      ...
      drawLabelList(params) {
        return pisSlideViewService.labelList(params)
          .then(({body}) => {
